"""
Privacy and Identifiability Analysis for MAPS framework.
...
class IdentifiabilityAnalyzer:
    ...
    2. Feature weighting (uniform in all reported experiments)
    ...
    def __init__(self, verbose: bool = True, max_distance_elements: int = 500_000_000):
        """
        Initialize the IdentifiabilityAnalyzer.
        
        Args:
            verbose: Whether to print progress information
            max_distance_elements: upper bound on the number of entries in one distance
                block, bounding peak memory independently of the pool size
        """
        self.verbose = verbose
        # Cap on elements held in one distance block. 5e8 float64 is about 4 GB, which keeps
        # the peak allocation predictable regardless of pool size.
        self.max_distance_elements = max_distance_elements
    ...
    def fit(self, 
        ...
        # Step 2: Feature weights (uniform; see _compute_feature_weights)
        if self.verbose:
    ...
    @staticmethod
    def _entropy_feature_weights(dataset: pd.DataFrame) -> np.ndarray:
        """Per-feature entropy, the weighting proposed with epsilon-identifiability.

        Provided for completeness. It is not what the reported experiments use --- see
        :meth:`_compute_feature_weights`.
        """
        X = np.asarray(dataset)
        return np.array([entropy(np.unique(np.round(X[:, i]), return_counts=True)[1])
                         for i in range(X.shape[1])])

    def _compute_feature_weights(self, dataset: pd.DataFrame) -> np.ndarray:
        """Uniform feature weights.

        The epsilon-identifiability formulation admits a per-feature weighting, and the
        reference implementation derives it from feature entropy. All results reported in
        the paper use uniform weights instead, so every feature contributes equally to the
        distance. The entropy variant remains available as
        :meth:`_entropy_feature_weights`.

        Note the scaling convention: distances are computed on ``x / w``, following the
        reference implementation, whereas the paper writes the weighting as ``w * x``.
        Under uniform weights the two coincide, which is why the reported numbers are
        unaffected; the distinction matters only if a non-uniform weighting is adopted.
        """
        return np.ones(np.asarray(dataset).shape[1])
    ...
    def _compute_identifiability_flags(self,
        ...
        # The full distance matrix is (n_real x n_synthetic). At the pool sizes used here
        # that does not fit in memory as one block -- 24,837 x 745,140 float64 is 138 GB for
        # GOSSIS -- so the synthetic axis is processed in chunks. The result is identical;
        # only the peak allocation changes, from the whole matrix to one chunk.
        chunk = max(1, int(self.max_distance_elements // max(no, 1)))
        if self.verbose and chunk < KN:
            print(f"  Computing distances in chunks of {chunk} synthetic samples "
                  f"({no} real x {KN} synthetic)")

        synthetic_identifiable_flags = np.zeros(KN, dtype=int)
        min_dist_real_to_syn = np.full(no, np.inf)
        thresholds = real_thresholds.reshape(-1, 1)
        n_real_violated_by_any = np.zeros(no, dtype=bool)

        for start in range(0, KN, chunk):
            stop = min(start + chunk, KN)
            block = cdist(X_hat, X_syn_hat[start:stop], metric='euclidean')
            violations = block < thresholds
            synthetic_identifiable_flags[start:stop] = np.any(violations, axis=0).astype(int)
            n_real_violated_by_any |= np.any(violations, axis=1)
            np.minimum(min_dist_real_to_syn, block.min(axis=1), out=min_dist_real_to_syn)
            del block, violations

        if self.verbose:
            print("  Distance computation complete. Finding privacy violations...")

        # Kept for the summary below; the full boolean matrix is never materialised.
        violation_matrix = n_real_violated_by_any
        distances_real_to_syn = min_dist_real_to_syn
        ...
        if self.verbose:
            ...
            real_samples_violated = int(np.sum(n_real_violated_by_any))
