...
#             # Also compute overall identifiability score for comparison
...
#         # Initialize identifiability flags for synthetic samples
#         synthetic_identifiable_flags = np.zeros(KN, dtype=int)
        
#         # For each real sample, find ALL synthetic samples that are closer than its r_i threshold
#         for real_idx in range(no):
#             real_sample = X_hat[real_idx:real_idx+1]  # Keep 2D shape for sklearn
#             real_threshold = real_thresholds[real_idx]
            
#             # Compute distances from this real sample to all synthetic samples
#             nbrs_syn = NearestNeighbors(n_neighbors=KN).fit(X_syn_hat)
#             distances_to_syn, syn_indices = nbrs_syn.kneighbors(real_sample)
            
#             # Find synthetic samples closer than the real threshold
#             violating_syn_mask = distances_to_syn[0] < real_threshold
#             violating_syn_indices = syn_indices[0][violating_syn_mask]
            
#             # Mark these synthetic samples as identifiable
#             synthetic_identifiable_flags[violating_syn_indices] = 1
            
#             # Progress update
#             if self.verbose and real_idx > 0 and real_idx % (no // 20 if no >= 20 else 1) == 0:
#                 print(f"  Processed {real_idx+1}/{no} real samples")
        
#         # Final progress update
#         if self.verbose:
#             print(f"  Finished processing {no}/{no} real samples")
