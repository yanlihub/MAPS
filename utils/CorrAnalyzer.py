...
#         measurements (array-like): Numerical variable
...
#         labs1 (list): First set of labels
#         labs2 (list): Second set of labels
    
#     Returns:
#         DataFrame: Result matrix
#     """
#     res = [func(data[lab1], data[lab2]) for lab1 in labs1 for lab2 in labs2]
#     return pd.DataFrame(
#         np.array(res).reshape(len(labs1), len(labs2)), 
...
#         method (str): Correlation method for numerical variables ('spearman', 'pearson', 'kendall')
    
#     Returns:
#         DataFrame: Mixed correlation matrix
#     """
#     # Correlation for numerical-numerical using specified method
#     if len(num_cols) > 0:
#         corr_num_num = data[num_cols].corr(method=method)
...
# def plot_correlation_heatmap(corr_matrix, title, figsize=(10, 8), annot=True, fmt=".2f"):
#     """
#     Plot correlation matrix heatmap with consistent color scale.
    
#     Args:
#         corr_matrix (DataFrame): Correlation matrix
#         title (str): Plot title
#         figsize (tuple): Figure size
#         annot (bool): Whether to show annotations
#         fmt (str): Number format for annotations
#     """
#     plt.figure(figsize=figsize)
#     mask = np.triu(np.ones_like(corr_matrix, dtype=bool), k=1) if len(corr_matrix) > 1 else None
    
#     # Set consistent color scale for all correlation matrices (-1 to 1)
#     sns.heatmap(
#         corr_matrix, 
#         annot=annot, 
#         cmap='coolwarm', 
...
# def plot_difference_heatmap(diff_matrix, title, figsize=(10, 8), annot=True, fmt=".3f"):
#     """
#     Plot correlation difference matrix heatmap.
    
#     Args:
#         diff_matrix (DataFrame): Difference matrix
#         title (str): Plot title
#         figsize (tuple): Figure size
#         annot (bool): Whether to show annotations
#         fmt (str): Number format for annotations
...
#     max_abs_val = np.abs(diff_matrix.values).max()
...
# ) -> Dict[str, Any]:
#     """
#     Comprehensive mixed correlation analysis for real and synthetic datasets.
    
#     Args:
#         real_data (DataFrame): Real dataset
#         raw_synthetic_data (DataFrame): Raw synthetic dataset
#         refined_synthetic_data (DataFrame): Refined synthetic dataset
#         num_cols (list, optional): List of numerical column names
#         cat_cols (list, optional): List of categorical column names
#         use_mixed (bool): Whether to use mixed correlation (True) or numerical only (False)
#         method (str): Correlation method for numerical variables ('spearman', 'pearson', 'kendall')
#         figsize (tuple): Figure size for plots
#         annot (bool): Whether to show annotations on heatmaps
#         show_plots (bool): Whether to display plots
    
#     Returns:
#         dict: Analysis results including correlation matrices, differences, and similarity scores
#     """
...
#     print(f"- Use mixed correlation: {use_mixed}")
#     print(f"- Correlation method for numerical variables: {method}")
#     print(f"- Numerical columns ({len(num_cols)}): {num_cols}")
#     print(f"- Categorical columns ({len(cat_cols)}): {cat_cols}")
#     print()
    
#     # Select columns based on use_mixed parameter
#     if use_mixed:
#         analysis_cols = num_cols + cat_cols
#         analysis_num_cols = num_cols
#         analysis_cat_cols = cat_cols
...
#     # Subset data to analysis columns
#     real_subset = real_data[analysis_cols].copy()
#     raw_subset = raw_synthetic_data[analysis_cols].copy()
#     refined_subset = refined_synthetic_data[analysis_cols].copy()
...
#     # Calculate global scale for difference matrices
#     global_max_abs_val = max(
#         np.abs(diff_raw.values).max(),
#         np.abs(diff_refined.values).max()
#     )
    
#     # Calculate similarity scores (lower is better)
#     score_raw = np.linalg.norm(diff_raw.values, ord='fro')
#     score_refined = np.linalg.norm(diff_refined.values, ord='fro')
    
#     # Normalize scores by matrix size for comparability
#     matrix_size = np.sqrt(corr_real.size)
#     score_raw_normalized = score_raw / matrix_size
#     score_refined_normalized = score_refined / matrix_size
    
#     # Print summary
#     print(f"=== {correlation_type} Analysis Results ===")
#     print(f"Matrix dimensions: {corr_real.shape}")
#     print(f"Raw Synthetic - Similarity Score: {score_raw_normalized:.4f}")
#     print(f"Refined Synthetic - Similarity Score: {score_refined_normalized:.4f}")
#     print(f"Improvement: {((score_raw_normalized - score_refined_normalized) / score_raw_normalized * 100):+.2f}%")
#     print()
    
#     # Plot correlation matrices with consistent color scale
#     if show_plots:
#         print("Plotting correlation matrices (all use the same color scale: -1 to +1)...")
#         plot_correlation_heatmap(
#             corr_real, 
#             f"Real Data - {correlation_type}", 
#             figsize, annot
#         )
        
#         plot_correlation_heatmap(
#             corr_raw, 
#             f"Raw Synthetic Data - {correlation_type}", 
#             figsize, annot
#         )
        
#         plot_correlation_heatmap(
#             corr_refined, 
#             f"Refined Synthetic Data - {correlation_type}", 
#             figsize, annot
#         )
        
#         # Plot difference matrices with consistent color scale
#         print("Plotting difference matrices (both use the same color scale for comparability)...")
        
#         # Plot first difference matrix
#         plt.figure(figsize=figsize)
#         sns.heatmap(
#             diff_raw, 
#             annot=annot, 
...
#             fmt=".3f",
#             center=0,
#             vmin=-global_max_abs_val,
#             vmax=global_max_abs_val,
#             square=True,
#             cbar_kws={"shrink": .8}
#         )
#         plt.title(f"Difference: Real vs Raw Synthetic - {correlation_type}", fontsize=14, pad=20)
#         plt.xticks(rotation=45, ha='right')
#         plt.yticks(rotation=0)
#         plt.tight_layout()
#         plt.show()
        
#         # Plot second difference matrix
#         plt.figure(figsize=figsize)
#         sns.heatmap(
#             diff_refined, 
#             annot=annot, 
...
#             cbar_kws={"shrink": .8}
#         )
#         plt.title(f"Difference: Real vs Refined Synthetic - {correlation_type}", fontsize=14, pad=20)
#         plt.xticks(rotation=45, ha='right')
#         plt.yticks(rotation=0)
#         plt.tight_layout()
#         plt.show()
...
#         },
#         'similarity_scores': {
#             'raw_synthetic': score_raw_normalized,
#             'refined_synthetic': score_refined_normalized,
#             'improvement_percentage': ((score_raw_normalized - score_refined_normalized) / score_raw_normalized * 100)
#         },
#         'configuration': {
#             'use_mixed': use_mixed,
#             'correlation_method': method,
...
#             'analysis_columns': analysis_cols,
#             'color_scale_info': {
#                 'correlation_matrices': 'Fixed scale: -1 to +1',
#                 'difference_matrices': f'Global scale: {-global_max_abs_val:.3f} to {global_max_abs_val:.3f}'
#             }
#         }
#     }
