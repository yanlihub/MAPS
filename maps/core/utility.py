"""
Utility Evaluation for MAPS Framework
...
def safe_train_test_split(data, test_size, stratify_on=None, random_state=42, what=""):
    """train_test_split that falls back to an unstratified split when stratification fails.

    Stratification needs at least two members of every class on each side. Rare categories
    routinely break that: TCGA's vital_status carries a single "not reported" record, and
    mode-collapsed generators can emit a class exactly once. Falling back keeps the run
    going and says so, rather than aborting the whole evaluation over one record.
    """
    if stratify_on is not None:
        counts = pd.Series(stratify_on).value_counts()
        rare = counts[counts < 2]
        if rare.empty:
            try:
                return train_test_split(
                    data, test_size=test_size, stratify=stratify_on, random_state=random_state
                )
            except ValueError as exc:
                warnings.warn(
                    f"Stratified split failed{' for ' + what if what else ''}: {exc}. "
                    "Falling back to an unstratified split.",
                    UserWarning, stacklevel=2,
                )
        else:
            warnings.warn(
                f"Stratified split not possible{' for ' + what if what else ''}: "
                f"class(es) {list(rare.index)} have fewer than 2 members. "
                "Falling back to an unstratified split.",
                UserWarning, stacklevel=2,
            )
...
def run_utility_evaluation(real_data, raw_synthetic_data, refined_synthetic_data, 
                          ...
                          real_train_data=None, real_test_data=None):
    """
    ...
    Args:
        ...
        refined_synthetic_data: Refined synthetic dataset (after MAPS)
        target_column: Name of the target column
        ...
        random_state: Random seed
        real_train_data: Optional. D_train from maps.data.splits. When given together with
            real_test_data, no internal split is performed and the strict data-isolation
            protocol is used: only these records are treated as available for training.
        real_test_data: Optional. D_test from maps.data.splits, held out from every stage of
            refinement. Required for the strict protocol.

    Notes:
        If real_train_data/real_test_data are omitted, the function falls back to splitting
        ``real_data`` internally. That path is retained for backward compatibility only: the
        resulting test set is a subset of whatever was passed as ``real_data``, so if the same
        frame was also used to fit the generator or MAPS, the evaluation is not held out.
        Use the strict protocol for any reported result.
    ...
    strict_protocol = real_train_data is not None and real_test_data is not None
    if (real_train_data is None) != (real_test_data is None):
        raise ValueError(
            "real_train_data and real_test_data must be supplied together"
        )

    if strict_protocol:
        # Strict data-isolation protocol: the partition was fixed once by
        # maps.data.splits.make_split and D_test never took part in refinement.
        print("Using STRICT data-isolation protocol (externally supplied D_train / D_test)")
        real_temp = real_train_data
        real_test = real_test_data
    else:
        warnings.warn(
            "run_utility_evaluation is splitting real_data internally. The resulting test "
            "set is a subset of real_data and is only held out if real_data was never used "
            "to fit the generator or MAPS. Pass real_train_data/real_test_data for the "
            "strict protocol.",
            UserWarning,
            stacklevel=2,
        )
        # First split: separate test data
        real_temp, real_test = safe_train_test_split(
            real_data, test_size=test_size,
            stratify_on=real_data[target_column], random_state=random_state,
            what="the real train/test split",
        )

    # Separate CV validation from training data
    real_train, real_cv_val = safe_train_test_split(
        real_temp, test_size=cv_val_size,
        stratify_on=real_temp[target_column], random_state=random_state,
        what="the real CV-validation split",
    )
...
def print_summary_table(results):
    ...
    print(f"Test Accuracy improvement from MAPS refinement: {test_acc_improvement:+.2f}%")
    print(f"Test F1-Macro improvement from MAPS refinement: {test_f1_improvement:+.2f}%")
    print(f"CV Accuracy improvement from MAPS refinement: {cv_acc_improvement:+.2f}%")
...
def run_clustering_evaluation(real_data, raw_synthetic_data, refined_synthetic_data,
    ...
    Args:
        real_data: Real dataset
        raw_synthetic_data: Original synthetic dataset
        refined_synthetic_data: Refined synthetic dataset (post MAPS)
        ...
        random_state: Random seed
        real_train_data: Optional D_train (strict data-isolation protocol; see
            run_utility_evaluation and maps.data.splits).
        real_test_data: Optional D_test, held out from every stage of refinement.
    ...
    strict_protocol = real_train_data is not None and real_test_data is not None
    if (real_train_data is None) != (real_test_data is None):
        raise ValueError("real_train_data and real_test_data must be supplied together")

    if strict_protocol:
        # Strict protocol: partition fixed once by maps.data.splits.make_split.
        print("Using STRICT data-isolation protocol (externally supplied D_train / D_test)")
        real_train = real_train_data[numeric_cols].fillna(0)
        real_test = real_test_data[numeric_cols].fillna(0)
        if use_true_labels:
            labels_train = real_train_data[target_column]
            labels_test = real_test_data[target_column]
        else:
            labels_train, labels_test = None, None
    elif use_true_labels:
        warnings.warn(
            "run_clustering_evaluation is splitting real_data internally; pass "
            "real_train_data/real_test_data for the strict protocol.",
            UserWarning, stacklevel=2,
        )
